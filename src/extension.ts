...
const languageServerPaths = [
    "server/EcmaLanguageServer.dll",
    "../../EcmaLanguageServer/bin/Debug/net5.0/EcmaLanguageServer.dll",
]
...
async function activateLanguageServer(context: vscode.ExtensionContext) {
    ...
    let clientOptions: LanguageClientOptions = {
        ...
        documentSelector: ["ecma5"],
        synchronize: {
            // Synchronize the setting section 'languageServerExample' to the server
            configurationSection: "ecmaLanguageServer",
            // Notify the server about file changes to '.clientrc files contain in the workspace
            fileEvents: [
                vscode.workspace.createFileSystemWatcher("**/.clientrc"),
                vscode.workspace.createFileSystemWatcher("**/.es5"),
            ]
        },
    }

    // Create the language client and start the client.
    client = new LanguageClient("ecmaLanguageServer", "ECMA 5 Language Server", serverOptions, clientOptions);
...
export async function activate(context: vscode.ExtensionContext) {
    console.log("ecma5 extension is now activated.");
